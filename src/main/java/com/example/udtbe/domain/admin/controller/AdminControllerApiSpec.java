...
public interface AdminControllerApiSpec {
    ...
    @Operation(summary = "콘텐츠 등록", description = "새로운 콘텐츠를 등록하고 ID를 반환합니다.")
    @ApiResponse(responseCode = "201", description = "등록된 콘텐츠 ID 반환")
    @PostMapping("/api/admin/contents")
    ResponseEntity<ContentRegisterResponse> registerContent(
            @RequestBody @Valid ContentRegisterRequest request
    );
    ...
    @PatchMapping("/api/admin/contents/{id}")
    ...
    @GetMapping("/api/admin/contents/{id}")
